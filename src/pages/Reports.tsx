...
import { diagnoses, monthlyStats } from '@/constants/storage';
...
const Reports = () => {
  ...
  const [recentReports, setRecentReports] = useState(diagnoses);
  ...
  const handleDownloadReport = (report: any) => {
    ...
    const reportContent = `
RURAL HEALTH AI AID - MEDICAL REPORT
===================================
...
DIAGNOSIS:
Based on the examination and symptoms presented, the patient shows signs of ${report.condition.toLowerCase()}. 
The condition appears to be ${report.status === 'Completed' ? 'successfully treated' : 'under active management'}.

TREATMENT RECOMMENDATIONS:
• Follow prescribed medication regimen as directed
• Schedule regular follow-up visits for monitoring
• Maintain proper hygiene and wound care if applicable
...
Generated by Rural Health AI Aid System
    ...
    // Create and download as text file (can be opened as PDF)
    const blob = new Blob([reportContent], { type: 'text/plain;charset=utf-8;' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Medical_Report_${report.patientName.replace(/\s+/g, '_')}_${report.date}.txt`;
    ...
    toast({
      ...
      description: `Report for ${report.patientName} has been downloaded`,
    });
  };
...
Rural Health AI Aid System
  ...
  const handleDownloadWeeklyReport = () => {
    const weeklyContent = `
RURAL HEALTH AI AID - WEEKLY SUMMARY REPORT
==========================================
...
- Fungal Infections: 5 cases
- Skin Rashes: 4 cases
- Minor Cuts: 3 cases

PERFORMANCE METRICS:
- Weekly Completion Rate: 67%
- Average Cases per Day: 1.7
...
Generated by Rural Health AI Aid System
    ...
    const blob = new Blob([weeklyContent], { type: 'text/plain;charset=utf-8;' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    ...
    toast({
      title: "Download Complete",
      description: "Weekly summary report has been downloaded",
    });
  };

  const handleShareWeeklyReport = () => {
    const message = `📊 *Weekly Health Summary*
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(message).then(() => {
        toast({
          ...
          description: "Weekly summary copied! You can paste it anywhere.",
        });
      }).catch(() => {
        alert('Weekly summary ready to share:\n\n' + message);
      });
    }
  };
...
RURAL HEALTH AI AID - MONTHLY ANALYTICS REPORT
==============================================
...
MONTHLY STATISTICS:
- Total Cases: 45
- Completed: 35 (78% success rate)
- Pending: 7 (15% of total)
- Referred: 3 (7% referral rate)
- Growth: +15% vs last month

TOP CONDITIONS THIS MONTH:
- Fungal Infections: 15 cases (33%)
- Contact Dermatitis: 10 cases (22%)
- Bacterial Infections: 8 cases (18%)
- Other conditions: 12 cases (27%)

PERFORMANCE TRENDS:
- Average Cases/Day: 2.3 ↑
- Diagnostic Accuracy: 94% ↑
- Follow-up Rate: 85% ↑
- Avg. Resolution Time: 3.2 days

VILLAGE BREAKDOWN:
- Rampur: 18 cases (Medium risk)
- Sundarpur: 15 cases (High risk)
- Greenfield: 8 cases (Low risk)
- Other villages: 4 cases

RECOMMENDATIONS:
- Continue focus on fungal infection prevention
- Increase follow-up rate in high-risk villages
- Maintain current diagnostic protocols

---
Generated by Rural Health AI Aid System
    `;

    const blob = new Blob([monthlyContent], { type: 'text/plain;charset=utf-8;' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Monthly_Analytics_${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
    ...
    toast({
      title: "Download Complete",
      description: "Monthly analytics report has been downloaded",
    });
  };

  const handleShareMonthlyReport = () => {
    const message = `📊 *Monthly Health Analytics*
...
Generated: ${new Date().toLocaleDateString()}`;
    ...
    if (navigator.share) {
      navigator.share({
        title: 'Monthly Health Analytics',
        text: message,
      }).catch(console.error);
    } else {
      navigator.clipboard.writeText(message).then(() => {
        toast({
          title: "Copied to Clipboard", 
          description: "Monthly analytics copied! You can paste it anywhere.",
        });
      }).catch(() => {
        alert('Monthly analytics ready to share:\n\n' + message);
      });
    }
  };
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-4">
              <Button className="flex-1" onClick={handleDownloadWeeklyReport}>
                <Download className="h-4 w-4 mr-2" />
                Download PDF
              </Button>
              <Button variant="outline" className="flex-1" onClick={handleShareWeeklyReport}>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2">
              <Button className="flex-1" onClick={handleDownloadMonthlyReport}>
                <Download className="h-4 w-4 mr-2" />
                Download Full Report
              </Button>
              <Button variant="outline" className="flex-1" onClick={handleShareMonthlyReport}>
                <Share className="h-4 w-4 mr-2" />
                Share Analytics
              </Button>
            </div>
